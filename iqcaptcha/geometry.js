...
const shapes = [
    ...
    },
    {
        ...
        drawin: function (ctx, res, fill = false) {
            ...
            ctx.bezierCurveTo(res / 5, 2 * res / 5, res - res / 5, 2 * res / 5, res - res / 8, res - res / 5);
            ctx.bezierCurveTo(res - res / 5, res / 5, res / 5, res / 5, res / 8, res - res / 5);
            ctx.closePath();
            _fillup(ctx, fill, res, this);
            ctx.restore();
        },
...
const drawLetter = (ctx, res, letter) => {
    ...
    if (TEXT_FILL) {
        ctx.fillStyle = LETTER_FILLSTYLE;
...
/**
 * @deprecated Use drawLetter instead.
 */
const drawQuestionMark = (ctx, res) => {
    ctx.save();
    ctx.translate(-res / 2, -res / 2);
    ctx.lineWidth = Math.max(res / 25, 1);
    ctx.beginPath();
    ctx.moveTo(2 * res / 6, res / 4);
    ctx.bezierCurveTo(2 * res / 6, 0, 4 * res / 6, 0, 4 * res / 6, res / 4);
    ctx.bezierCurveTo(res / 2, res / 3, res / 2, res / 3 + res / 20, res / 2, 3 * res / 5);
    ctx.stroke();
    ctx.closePath();
    ctx.beginPath();
    ctx.arc(res / 2, 4 * res / 5, Math.max(1, res / 25), 0, Math.PI * 2);
    ctx.fill();
    ctx.closePath();
    ctx.restore();
};
...
const deciders = {
    ...
    decideParams: ovds => {
        ...
        for (let i of [0, 1]) {
            if (res[i].fill && ovds[i] === painters.insert) {
                if (ovds[i + 1] === painters.insert || ovds[Math.min(i + 2, 2)] === painters.insert)
                    if ((res[i].tx === res[i + 1].tx && res[i].ty === res[i + 1].ty) || (res[i].tx === res[Math.min(i + 2, 2)].tx && res[i].ty === res[Math.min(i + 2, 2)].ty))
                        res[i].fill = false;
            }
        }
        ...
        if (ROTATE_PER_LAYER) {
            const baseAngle = random.randInt(10, 40),
                perLayerRotations = [baseAngle, ...random.genDistinct(10, 45, 2).sort().map(val => val + baseAngle)];
            res.forEach((item, i) => {
    ...
    decideOptions: (qShapeIndices, ovds, ovdParams, resolutions) => {
         ...
         */
        const letters = random.shuffle(POSSIBLE_LETTERS).slice(0, 10);
        const correctAnswerLetters = [];
        {
            const correctShapeIndices = [qShapeIndices[2], random.genDistinct(0, shapes.length - 1, 1, qShapeIndices)[0]],
...
/**
 * Number randomizers. Uses sin seed function if debugging {@link _dbg} is on.
 */
const random = {
    rnd: () => {
        if (!_dbg)
        ...
        if (!random.rnd.seed)
            random.rnd.seed = 0;
        random.rnd.seed += 3;
        const x = Math.sin(random.rnd.seed) * 10000;
...
const drawGroup = (context, providedShapes, providedFunctions, providedParams, resolutions) => {
    ...
    providedShapes.forEach((shape, idx) => {
        context.save();
        [...Array(providedShapes.length).keys()].forEach(i => context.rotate(_dgtorad(providedParams[i].layerRotation || 0)));
...
module.exports = {shapes, painters, deciders, drawGroup, drawQuestionMark, drawLetter, random};