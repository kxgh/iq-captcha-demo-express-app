...
class CaptchaAuthr {
     ...
     * @param {IQC} provider object with async CAPTCHA provider function popCaptcha
     ...
     * @param {number} [authPreferences.requiredAnswers] the number of answers required. Default 3
     * @param {boolean} [authPreferences.resetOnWrong] whether the correct answer count resets on wrong answer. Default true
     * @param {number} [authPreferences.answerTimeout] the time limit in milliseconds the subject has for one answer. Default 25000
    ...
    constructor(provider, authPreferences = {}) {
        ...
        this.regenRequested = ans => (ans == 'regen');
        this.genReturnValue = (record, state, withQ) => {
            const c = {
                state: state,
                info: record.getInfo()
            };
    ...
    async deAuthAndGenNew(id) {
        ...
        await r.genCaptcha();
        this.map.set(id, r);
        return this.genReturnValue(r, 'new', true);
    }
    ...
    async _handleRegen(rec, id) {
        if (rec) {
            ...
            if (rec.isLimited())
                return this.genReturnValue(rec, 'limit');
            await rec.genCaptcha();
            return this.genReturnValue(rec, 'new', true);
        }
        ...
        if (rec) {
            ...
            if (rec.tookTooLong()) {
                ...
                if (rec.isLimited())
                    return this.genReturnValue(rec, 'limit');
                await rec.genCaptcha();
                return this.genReturnValue(rec, 'timeout', true);
            }
            if (rec.checkAnswer(ans) && !rec.wasTooFast()) {
                rec.correct++;
                if (rec.authd) {
                    return this.genReturnValue(rec, 'success');
                } else {
                    await rec.genCaptcha();
                    return this.genReturnValue(rec, 'more', true);
                }
            } else {
                rec.wrong++;
                ...
                if (rec.isLimited())
                    ...
                    await rec.genCaptcha();
                    return this.genReturnValue(rec, 'wrong', true);
                }
            }
